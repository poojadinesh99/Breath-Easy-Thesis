...
class SupabaseService:
    ...
    async def save_analysis_result(
        ...
        try:
            ...
            if result.data:
                ...
                logger.info(f"✓ Analysis result saved to Supabase (ID: {record_id})")
                return {
                    "status": "success",
                    "saved": True,
                    "record_id": record_id,
    ...
    async def create_alert_if_needed(
        ...
        """
        if not self.connected:
            logger.info("📴 Database offline - alert not created")
            return None
        ...
        try:
            ...
            if alert_type and alert_message:
                ...
                if result.data:
                    ...
                    logger.info(f"🚨 Alert created: {alert_type} (ID: {alert_id})")
                    return {
                        "status": "created",
                        "alert_id": alert_id,
                        "alert_type": alert_type,
                    ...
                    logger.warning("⚠️ Alert creation returned no data")
                    return {"status": "failed", "error": "No data returned"}
            else:
                logger.info("✅ No alert needed - results within normal parameters")
